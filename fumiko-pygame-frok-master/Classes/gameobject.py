...
from Classes.common import random_id, Position, Size
...
class GameObject:
    def __init__(self, name=None, position=None, size=None, image=None):
        ...
        def define_image():
          if not image:
              self.image = Classes.images.get_image(Classes.images.resources["empty"])
          else:
              self.image = image
        ...
        def add_physics_controller():
            self.physics = Classes.physics.PhysicsController2D(self.rect)
    ...
    def chimg(self, img):
        self.image = Classes.images.get_image(Classes.images.resources[img])
